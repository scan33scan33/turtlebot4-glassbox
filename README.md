...
| `push_ball_to_wall` | find a ball, centre it, then make short camera-steered shoves toward the wall (A* approaches only when far away). No goal needed — **the default.** |
...
### If it finds the ball but doesn't push

`push_ball_to_wall` used to hand a **single, frozen ball position** to A* as a
push goal. A camera scan could succeed while the navigator waited for lidar,
failed to plan into a nearby ball, or drove to a noisy/old depth fix rather than
making contact. Close up, `PUSH_AWAY` now keeps the **live ball bearing** centred
and drives a bounded shove; for a distant ball it still plans to a standoff
first. The program stops with `lost` or `blocked` instead of blind-pushing or
claiming success if that approach fails. Only a ball last seen centred at the
bumper permits the short `PUSH_THROUGH` finish.

If it still finds but doesn't **move**, read **Status**, the program's **Result**,
the **Last /cmd_vel** readout and the health dots. A red `detector` means NN
detections are stale (a green `cam` only means raw video is alive); distant
approaches require green `lidar`. If **while PUSH_AWAY is running** the command
shows `v > 0` but Pos X/Y do not change, check Create 3 `/wheel_status`
(`wheels_enabled`) and the ROS bridge as described in
[operations](docs/operations.md) — software cannot override disabled wheels.
**The revised wall-push controller has only been simulation-tested; try it
supervised on an open, flat floor before relying on it.**
...
`cam · detector · lidar · base` dots; **4 green = go**. The detector dot is
separate because the camera may stream pictures even when its NN stops sending
ball detections. Undock, then run a behavior.
...
- **Health badge:** `cam_ok`/`nn_ok`/`lidar_ok`/`base_ok` in `/state` surface
  camera, detector, lidar and odometry freshness so a problem is visible before
  a run.
...
python3 -m unittest tests.test_push_to_goal tests.test_push_to_wall   # closed-loop ROS-stubbed tests
...
- `tests/test_push_to_goal.py` / `tests/test_push_to_wall.py` — closed-loop
  tests for the ball-push geometry, wall and goal skills (ROS stubbed, no hardware needed)
